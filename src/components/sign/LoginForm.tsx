...
export default function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!profileData) {
      //console.log('[프로필 없음] → 닉네임 등록 페이지로 이동');
      router.push('/sign/register-profile');
    } else {
      //console.log('[프로필 있음] → 홈으로 이동');
      router.push('/');
    }
  ...
  return (
    ...
    >
      ...
      <div className="container_lc">
        <label htmlFor="password" className="padL block text-sm font-semibold text-var(--color-primary-dark) mb-2">
          * 비밀번호
        </label>
        <input
          id="password"
