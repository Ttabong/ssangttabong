    ...
    <main>
        ...
        <div className="flex justify-center items-start min-h-[600px] px-4 py-8 bg-[#0f172a]">
            <div className="flex flex-col md:flex-row w-full max-w-6xl gap-8 mt-10">
                ...
                {/* 설명 */}
                <section className="w-full border border-white rounded-md text-white flex-1 h- overflow-auto" style={{ paddingLeft: '1rem', paddingRight: '1rem'}}>
                ...
                <div className="w-full md:w-1/2 bg-white/10 backdrop-blur-sm p-6 rounded-md shadow text-gray-100 border border-white flex flex-col gap-6">
                    {/* 제목 */}
                    <h1 className="text-2xl md:text-3xl font-bold text-orange-400 border-b border-gray-500 pb-2" style={{ paddingLeft: '1rem', paddingRight: '1rem', paddingTop: '1rem',minHeight: '70px' }}>
                        ○ {data.title}
                    </h1>

                    {/* 상세 정보 */}
                    <div className="grid grid-cols-2 gap-5 text-xl" style={{ paddingLeft: '1rem', paddingRight: '1rem'}}>
                        {details.map(([label, value], i) => (
                        <div
                        key={i}
                        className={`flex justify-between items-center rounded-md ${
                            i % 2 === 0 ? 'bg-white/5' : 'bg-white/5'
                        }`}
                        style={{ paddingLeft: '1rem', paddingRight: '1rem', paddingTop: '0.5rem', paddingBottom: '0.5rem' }} // 인라인 스타일로 좌우 패딩 강제 적용
                        >
